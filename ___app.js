...
class BeatSaberGame {
    constructor() {
        ...
        this.vrEntryEl = document.getElementById('vr-entry');

        // 5. Contexto de Audio Web
        this.audioCtx = null;
        ...
        this.renderer.setAnimationLoop((time, frame) => this.onAnimationFrame(time, frame));
    }

    setupWebXR() {
        if (this.vrEntryEl) {
            const vrButton = VRButton.createButton(this.renderer);
            this.vrEntryEl.appendChild(vrButton);
        }

        this.renderer.xr.setReferenceSpaceType('local-floor');

        this.controllerGrips = [null, null];
        this.gripHandedness = new Map();

        for (let i = 0; i < 2; i++) {
            const grip = this.renderer.xr.getControllerGrip(i);
            grip.addEventListener('connected', (event) => this.onControllerConnected(i, event));
            grip.addEventListener('disconnected', () => this.gripHandedness.delete(i));
    ...
    onControllerConnected(index, event) {
        const handedness = event.data.handedness;
        if (handedness !== 'left' && handedness !== 'right') return;

        this.gripHandedness.set(index, handedness);
        this.attachSaberToGrip(index, handedness);
    }

    attachSaberToGrip(index, handedness) {
        const saber = handedness === 'left' ? this.leftSaber : this.rightSaber;
        const grip = this.controllerGrips[index];
        if (!saber || !grip) return;

        if (saber.group.parent) {
            saber.group.parent.remove(saber.group);
        }

        grip.add(saber.group);
        this.applyVRSaberLocalTransform(saber);
        saber.vrGripIndex = index;
        saber.vrHandedness = handedness;
    }

    applyVRSaberLocalTransform(saber) {
        saber.group.position.set(0, 0, -0.06);
        saber.group.rotation.set(-Math.PI / 3, 0, 0);
    }

    onXRSessionStart() {
        this.isVR = true;
        this.gameRoot.position.z = this.vrWorldOffsetZ;
        ...
        if (session) {
            session.addEventListener('inputsourceschange', () => this.syncVRSabersFromInputSources());
            this.syncVRSabersFromInputSources();
        }
    }

    syncVRSabersFromInputSources() {
        for (let i = 0; i < 2; i++) {
            const handedness = this.gripHandedness.get(i);
            if (handedness) {
                this.attachSaberToGrip(i, handedness);
            }
        }
    }
    ...
    onXRSessionEnd() {
        this.isVR = false;
        this.gameRoot.position.z = 0;
        this.gripHandedness.clear();
        this.restoreDesktopSabers();
    }

    restoreDesktopSabers() {
        const restore = (saber, side) => {
            if (saber.group.parent) {
                saber.group.parent.remove(saber.group);
            }
            this.scene.add(saber.group);
            saber.group.rotation.set(-Math.PI / 4, 0, 0);
            saber.group.position.copy(saber.currentPos);
            saber.vrGripIndex = undefined;
            saber.vrHandedness = undefined;
            if (side === 'left') {
                saber.targetPos.set(-0.45, 1.2, this.hitZ);
            } else {
                saber.targetPos.set(0.45, 1.2, this.hitZ);
            }
        };

        restore(this.leftSaber, 'left');
        restore(this.rightSaber, 'right');
    }
    ...
    setupSabers() {
        ...
        this.leftSaber.targetPos.set(-0.45, 1.2, this.hitZ);
        this.leftSaber.currentPos.copy(this.leftSaber.targetPos);
        this.leftSaber.group.position.copy(this.leftSaber.currentPos);
        this.scene.add(this.leftSaber.group);

        this.rightSaber = this.createSaber(0x00f3ff);
        this.rightSaber.targetPos.set(0.45, 1.2, this.hitZ);
        this.rightSaber.currentPos.copy(this.rightSaber.targetPos);
        this.rightSaber.group.position.copy(this.rightSaber.currentPos);
    ...
    setupPointerControls() {
        ...
        window.addEventListener('touchmove', (event) => {
            ...
            if (event.touches.length >= 2) {
                ...
                this.leftSaber.targetPos.set(x0 * 2.2, 1.3 + y0 * 1.0, this.hitZ);

                const t1 = event.touches[1];
                const x1 = (t1.clientX / window.innerWidth) * 2 - 1;
                const y1 = -(t1.clientY / window.innerHeight) * 2 + 1;
                this.rightSaber.targetPos.set(x1 * 2.2, 1.3 + y1 * 1.0, this.hitZ);
    ...
    checkCollisions() {
        ...
        for (let i = this.blocks.length - 1; i >= 0; i--) {
            ...
            if (distToSaber < 0.38) {
                ...
                this.gameRoot.remove(block.mesh);
    ...
    updateSaber(saber) {
        ...
        if (this.isVR) {
            saber.group.getWorldPosition(saber.currentPos);
        } else {
        ...
        const targetRotX = (this.isVR ? -Math.PI / 3 : -Math.PI / 4) + saber.velocity.y * 2.0;

        if (!this.isVR) {
            saber.group.rotation.z += (targetRotZ - saber.group.rotation.z) * 0.2;
            saber.group.rotation.x += (targetRotX - saber.group.rotation.x) * 0.2;
        } else {
            saber.group.rotation.z += (targetRotZ - saber.group.rotation.z) * 0.15;
        }
    }
